...
import KhulnaSoftLogo from "img/openSauced-icon.png";
...
const footerContext = [
  {
    ...
    status: { url: "http://status.khulnasoft.com/", text: "Status" },
  },
  {
  ...
  },
  {
    socials: [
      {
        url: "https://twitter.com/khulnasoftopen",
      ...
      {
        url: "https://discord.com/invite/58KbV5w2",
      ...
      <div className=" container mx-auto px-2 md:px-16  lg:border-t lg:py-8 lg:items-center lg:justify-between lg:gap-x-4 flex flex-col gap-y-4 lg:flex-row py-2 w-full">
        <div className="text-center lg:text-left justify-center gap-1 flex items-center">
          <div className="w-6 h-6 relative !min-w-[24px] min-h-[24px]">
            <Image fill={true} alt="brand logo" src={KhulnaSoftLogo} />
          </div>
          ...
          <Text className="hidden !text-light-slate-9 lg:inline-block">
            © {new Date().getFullYear()} <span className="hidden lg:inline-block">KhulnaSoft</span>
          </Text>
        </div>
        ...
        <div className="flex md:justify-center lg:hidden lg:border-none lg:order-2 border-t py-3 pb-4 mt-2 text-sm gap-x-14">
          <Text className="text-light-slate-9">
            © {new Date().getFullYear()} <span className="hidden md:inline-block">Open khulnasoft</span>
          </Text>
