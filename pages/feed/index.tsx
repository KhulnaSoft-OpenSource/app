...
const Feeds: WithPageLayout<HighlightSSRProps> = (props: HighlightSSRProps) => {
  ...
  if (!hydrated)
    return (
      <>
        <SEO
          title={`${props?.highlight ? "Highlight | KhulnaSoft" : "Highlights | KhulnaSoft"}`}
          description={`${props?.highlight?.highlight || "KhulnaSoft Highlight"}`}
          image={ogImage}
      ...
      >
        <div className={`sticky ${user ? "top-16" : "top-8"} xl:flex hidden flex-none w-[22%]`}>
          <div className="flex flex-col w-full gap-6 mt-12">
            ...
            <AnnouncementCard
              ...
              url={"https://dev.to/khulnasoft/100daysofoss-growing-skills-and-real-world-experience-3o5k"}
            />
          </div>
        </div>
        {singleHighlight && (
        ...
        <Tabs onValueChange={onTabChange} defaultValue="home" className="grow">
          <TabsList className={clsx("justify-start  w-full border-b", !user && "hidden")}>
            <TabsTrigger
              className="data-[state=active]:border-khulnasoft-orange data-[state=active]:border-b-2 text-2xl"
              value="home"
            >
              Home
            </TabsTrigger>
